...
public abstract class TestBase {
    ...
    protected FastCacheAsyncClient client;
    ...
    void setUp() throws IOException {
        ...
        if (Boolean.getBoolean(property)){
            ...
            server = InProcessServerBuilder.forName(serverName)
                    .addService(new MockFastCacheService())
                    .build()
                    .start();
            client = new FastCacheAsyncClient(InProcessChannelBuilder.forName(serverName).build());
        }else {
            client = new FastCacheAsyncClient("127.0.0.1", 50000, Duration.ofSeconds(3600));
        }
    ...
    public static class MockFastCacheService extends FastCacheGrpcServiceGrpc.FastCacheGrpcServiceImplBase {
        ...
        private Value createValue(byte[] data) {
            return Value.newBuilder()
                    .setValue(BinaryPayload.newBuilder().setPayload(com.google.protobuf.ByteString.copyFrom(data)).build())
                    .build();
        }
        ...
        public void updateValue(UpdateRequest request, StreamObserver<UpdateValueResponse> responseObserver) {
            ...
            responseObserver.onNext(UpdateValueResponse.newBuilder()
                    .setResult(true)
                    .setValue(oldValue != null ? oldValue : Value.getDefaultInstance())
                    .build());
            responseObserver.onCompleted();
        }
